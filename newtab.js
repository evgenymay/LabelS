const UI_LOCALE_KEY = 'preferred_ui_locale';
let localeCatalog = null;
let effectiveUiLang = 'en';

function resolveEffectiveLang(pref) {
...
function formatCatalogMessage(entry, substitutions) {
  ...
  if (entry.placeholders && typeof entry.placeholders === 'object') {
    Object.entries(entry.placeholders).forEach(([name, spec]) => {
      const content = spec && spec.content ? String(spec.content).trim() : '';
      const m = /^\$(\d+)$/.exec(content);
      const idx = m ? parseInt(m[1], 10) - 1 : -1;
      const val = idx >= 0 && subs[idx] !== undefined ? String(subs[idx]) : '';
      const esc = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      msg = msg.replace(new RegExp('\\$' + esc + '\\$', 'gi'), val);
    });
  }
  return msg;
}
...
function __(key, substitutions) {
  if (localeCatalog && localeCatalog[key]) {
    const out = formatCatalogMessage(localeCatalog[key], substitutions);
    if (out && out.length) return out;
  }
  try {
    const msg = chrome.i18n.getMessage(key, substitutions);
    return msg && msg.length ? msg : key;
  } catch (e) {
    return key;
  }
}
...
function applyDataI18n(root = document) {
  ...
  const titleHolder = document.getElementById('doc-page-title');
  const pt = __('pageTitle');
  if (titleHolder) titleHolder.textContent = pt;
  document.title = pt;
  document.documentElement.lang = effectiveUiLang;
}

const _storedUiPref = await new Promise((resolve) => {
  chrome.storage.sync.get(UI_LOCALE_KEY, resolve);
});
effectiveUiLang = resolveEffectiveLang(_storedUiPref[UI_LOCALE_KEY]);
try {
  await loadLocaleCatalog(effectiveUiLang);
} catch (e) {
  console.warn('[LabelS] locale catalog load failed:', effectiveUiLang, e);
  localeCatalog = null;
}
...
function showNoSpacePopup(anchorEl) {
  ...
  if (!popup) {
    ...
    `;
    popup.textContent = __('noSpaceOnScreen');
  ...
  getStoredSavedDomains((saved_domains) => {
    getStoredSettings((settings) => {
      ...
      if (existing && existing.visible) {
        ...
        if (!fromPopup) {
          showLightNotice(__('noticeDuplicateSite'), noticePoint);
        }
...
function syncLocaleSegmentButtons() {
  document.querySelectorAll('.locale-segment').forEach((btn) => {
    const on = btn.dataset.locale === effectiveUiLang;
    btn.classList.toggle('active', on);
    btn.setAttribute('aria-pressed', on ? 'true' : 'false');
  });
}
syncLocaleSegmentButtons();
document.querySelectorAll('.locale-segment').forEach((btn) => {
  btn.addEventListener('click', () => {
    const v = btn.dataset.locale;
    if (v !== 'ru' && v !== 'en') return;
    if (v === effectiveUiLang) return;
    chrome.storage.sync.set({ [UI_LOCALE_KEY]: v }, () => location.reload());
  });
});
...
function autoSavedProfileDisplayName() {
  return __('profileAutoSaved');
}
...
function normalizeProfileDomainsForCompare(domains) {
  return (domains || [])
    .filter(d => d && d.domain)
    .map(d => ({
      domain: d.domain,
...
function areDomainLayoutsEqual(a, b) {
  const left = normalizeProfileDomainsForCompare(a);
  const right = normalizeProfileDomainsForCompare(b);
...
function resetProfilePopupTitle() {
  setProfilePopupTitle(__('profileNameDialogTitle'), false);
}

function showDuplicateProfileAlert(profileName) {
  setProfilePopupTitle(__('profileDuplicateTitle', [profileName]), true);
...
function extractDomainsForProfile(domains) {
  return (domains || [])
    .filter(d => d && d.domain)
    .map(d => ({
      domain: d.domain,
...
function getCurrentProfileState(cb) {
  getSyncedConfig(['saved_domains', 'manualLayout'], ({ saved_domains, manualLayout }) => {
    cb({
      ...
      domains: extractDomainsForProfile(saved_domains),
      manualLayout: !!manualLayout
    });
  });
}
    ...
    if (!currentState.manualLayout && areSettingsEqual(currentState.settings, DEFAULT_SETTINGS)) {
      getDefaultProfileState((defaultState) => {
        if (
          ...
          areDomainLayoutsEqual(defaultState.domains, currentState.domains.filter(d => d.visible))
        ) {
...
function autoSaveLastProfile(cb) {
  getCurrentProfileState((state) => {
    const profileData = {
      name: autoSavedProfileDisplayName(),
    ...
    getProfileIndex((index) => {
      ...
      if (!hasLast) {
        index.push({ name: autoSavedProfileDisplayName(), key: LAST_PROFILE_KEY });
      }
...
function updateProfileActionButton(selectedValue) {
  ...
  actionBtn.textContent = __('btnDelete');
...
function updateProfilePanelButtons() {
  const renameBtn = document.getElementById('panel-profile-rename');
  const deleteBtn = document.getElementById('panel-profile-delete');
  if (!renameBtn || !deleteBtn || !profileSelect) return;
  const v = profileSelect.value;
  const isSaved = v && v !== DEFAULT_PROFILE_VALUE && v !== CUSTOM_PROFILE_VALUE;
  renameBtn.disabled = !isSaved;
  deleteBtn.disabled = !isSaved;
}
...
function refreshProfileSelect(selectedValue) {
  ...
  getStoredProfileSnapshot((savedProfileSnapshot) => {
    getProfileIndex((index) => {
      ...
      defaultOption.textContent = __('profileDefault');
...
function loadSavedProfile(profileKey) {
  chrome.storage.sync.get(profileKey, (result) => {
    ...
    const savedDomains = domains.map((item, index) => ({
      ...
      date_added: item.date_added != null ? item.date_added : Date.now() + index
    }));
    const snapshot = {
...
function startRename(profileKey) {
  getProfileIndex((index) => {
    ...
    if (profilePopupTitle) {
      profilePopupTitle.textContent = __('profileRenameTitle');
  ...
  getProfileIndex((index) => {
    ...
    function afterRenamePersist(selectKey) {
      ...
      setTimeout(() => {
        profilePopup.classList.remove('open');
        resetProfilePopupTitle();
        refreshProfileSelect(selectKey);
        updateDefaultBtnState();
    ...
    if (key === LAST_PROFILE_KEY) {
      chrome.storage.sync.get(key, (result) => {
        ...
        const hasLastInIndex = index.some((p) => p.key === LAST_PROFILE_KEY);
        const newIndex = hasLastInIndex
          ? index.map((p) => (p.key === LAST_PROFILE_KEY ? { name, key: newKey } : p))
          : index.concat([{ name, key: newKey }]);
        chrome.storage.sync.set(
          {
            [newKey]: profileData,
          ...
          () => {
            chrome.storage.sync.remove(LAST_PROFILE_KEY, () => {
              afterRenamePersist(newKey);
            });
          }
        );
      });
...
function applyImportedBackup(backupText, onDone) {
  ...
  try {
    ...
    alert(__('importErrBadFormat'));
    if (typeof onDone === 'function') onDone(false);
    return;
  }
  if (!backup.local || !backup.sync) {
    alert(__('importErrMissingSyncLocal'));
    if (typeof onDone === 'function') onDone(false);
    return;
  }
  ...
  if (firstProfileMeta) {
    ...
    const snapshot = {
      name: firstProfile.name || firstProfileMeta.name || __('fallbackImportProfileName'),
      settings,
...
const panelProfileRename = document.getElementById('panel-profile-rename');
const panelProfileDelete = document.getElementById('panel-profile-delete');
const panelProfileImport = document.getElementById('panel-profile-import');
const panelProfileExport = document.getElementById('panel-profile-export');
if (panelProfileRename) {
  panelProfileRename.addEventListener('click', () => {
    if (panelProfileRename.disabled) return;
    hideProfileContextMenu();
    startRename(profileSelect.value);
  });
}
if (panelProfileDelete) {
  panelProfileDelete.addEventListener('click', () => {
    if (panelProfileDelete.disabled) return;
    hideProfileContextMenu();
    if (!confirm(__('ctxConfirmDelete'))) return;
    deleteSavedProfile(profileSelect.value);
  });
}
if (panelProfileImport) {
  panelProfileImport.addEventListener('click', () => {
    hideProfileContextMenu();
    importData();
  });
}
if (panelProfileExport) {
  panelProfileExport.addEventListener('click', () => {
    hideProfileContextMenu();
    exportData();
  });
}
...
function showNoSpaceAlert(totalBytes, usedBytes, profileBytes) {
  ...
  title.style.cssText = 'font-size:16px;font-weight:600;color:#dc2626;margin-bottom:10px';
  title.textContent = __('quotaAlertTitle');

  const body = document.createElement('div');
  body.style.cssText = 'text-align:left;line-height:1.8';
  const pushLine = (msg) => {
    body.appendChild(document.createTextNode(msg));
    body.appendChild(document.createElement('br'));
  };
  pushLine(__('quotaLineTotal', [(totalBytes / 1024).toFixed(1)]));
  pushLine(__('quotaLineUsed', [(usedBytes / 1024).toFixed(1)]));
  pushLine(__('quotaLineFree', [(freeBytes / 1024).toFixed(1)]));
  body.appendChild(document.createTextNode(__('quotaLineProfile', [(profileBytes / 1024).toFixed(1)])));
...
function renderList(domains) {
  ...
  if (!domains || domains.length === 0) {
    ...
    emptyP.textContent = __('emptyLabelsHint');
    out.appendChild(emptyP);
    return;
  }
  out.replaceChildren();
  const table = document.createElement("table");
  const headerRow = document.createElement("tr");
  ['tableShow', 'tableDomain', 'tableVisits'].forEach((key) => {
    const th = document.createElement('th');
    th.textContent = __(key);
...
const ENGINES = {
  ...
  yandex: { action: 'https://yandex.ru/search/', param: 'text' }
};

function searchPlaceholderForEngine(engineKey) {
  return engineKey === 'yandex' ? __('searchPlaceholderYandex') : __('searchPlaceholderGoogle');
}
...
function applySearchEngine(engine, persist = true) {
  ...
  searchInput.placeholder = searchPlaceholderForEngine(engine);
